...
class MonitorProcesos:
    def __init__(self, ventana):
        ...
        self.lista_sjf_en_ejecucion = None
        self.lista_sjf_cola = None
        self.boton_mostrar_sjf = None

        self.crear_marco_sjf() 
    ...
    def crear_marco_sjf(self):
        marco_sjf = ttk.Frame(self.ventana)
        marco_sjf.pack(side=tk.RIGHT, padx=10, pady=10)
        etiqueta_sjf = ttk.Label(marco_sjf, text="Algoritmo SJF", font=('Helvetica', 12, 'bold'))
        etiqueta_sjf.pack()

        columnas_sjf_en_ejecucion = ("Información")
        self.lista_sjf_en_ejecucion = ttk.Treeview(marco_sjf, columns=columnas_sjf_en_ejecucion, show="headings")
        self.lista_sjf_en_ejecucion.heading("Información", text="Procesos en Ejecución")
        self.lista_sjf_en_ejecucion.column("Información", width=300)
        self.lista_sjf_en_ejecucion.config(height=10)
        self.lista_sjf_en_ejecucion.pack()

        columnas_sjf_cola = ("Información")
        self.lista_sjf_cola = ttk.Treeview(marco_sjf, columns=columnas_sjf_cola, show="headings")
        self.lista_sjf_cola.heading("Información", text="Cola de Procesos")
        self.lista_sjf_cola.column("Información", width=300)
        self.lista_sjf_cola.config(height=10)
        self.lista_sjf_cola.pack()

        self.boton_mostrar_sjf = ttk.Button(marco_sjf, text="Mostrar SJF", command=self.alternar_sjf)
        self.boton_mostrar_sjf.pack()
        
    def imprimir_sjf(self, procesos):
        # Implementa el algoritmo SJF aquí
        procesos_sjf = sorted(procesos, key=lambda x: x['cpu_percent'])

        # Muestra los procesos en ejecución
        for proceso in procesos_sjf[:5]:
            pid = proceso['pid']
            nombre = proceso['name']
            self.lista_sjf_en_ejecucion.insert("", "end", values=(f"PID: {pid}, Nombre: {nombre}"))

        # Muestra la cola de procesos
        for proceso in procesos_sjf[5:]:
            pid = proceso['pid']
            nombre = proceso['name']
            self.lista_sjf_cola.insert("", "end", values=(f"PID: {pid}, Nombre: {nombre}"))

    def alternar_sjf(self):
        if self.boton_mostrar_sjf.cget("text") == "Mostrar SJF":
            self.ocultar_algoritmos()
            procesos = [p.info for p in psutil.process_iter(['pid', 'name', 'cpu_percent', 'nice'])]
            procesos_importantes = [p for p in procesos if p['cpu_percent'] > 5 and p['pid'] not in self.procesos_impresos and p['name'] not in self.procesos_a_excluir]
            self.imprimir_sjf(procesos_importantes)
            self.lista_sjf_en_ejecucion.pack()
            self.lista_sjf_cola.pack()
            self.boton_mostrar_sjf.config(text="Ocultar SJF")
        else:
            self.lista_sjf_en_ejecucion.pack_forget()
            self.lista_sjf_cola.pack_forget()
            self.boton_mostrar_sjf.config(text="Mostrar SJF")
